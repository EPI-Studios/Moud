...
public final class RopeCurve {
    ...
    public record Link(Vector3 position, Vector3 forward, Vector3 up, double length) {}
    ...
    public static List<Link> links(List<Vector3> points, double spacing, double twist, int most) {
        ...
        if (points.size() < 2 || !(spacing > 0) || most <= 0) return links;
        ...
        if (total < 1.0e-9) return links;
        int count = (int) Math.min(most, Math.max(1, Math.round(total / spacing)));
        double step = total / count;
        Vector3 up = null;
        Vector3 forward = null;
        int segment = 1;
        for (int k = 0; k < count; k++) {
            double along = step * (k + 0.5);
            ...
            if (span > 1.0e-9) forward = b.sub(a).mul(1 / span);
            if (forward == null) continue;
            Vector3 position = span > 1.0e-9 ? a.add(forward.mul(along - reach[segment - 1])) : a;
            ...
            links.add(new Link(position, forward, rolled, step));
        }
        return links;
    }
}
