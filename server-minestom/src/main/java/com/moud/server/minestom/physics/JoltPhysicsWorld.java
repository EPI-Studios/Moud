...
public final class JoltPhysicsWorld implements PhysicsWorld {
    ...
    private final com.github.stephengold.joltjni.JobSystemSingleThreaded jobs =
            new com.github.stephengold.joltjni.JobSystemSingleThreaded(1024);
    ...
    public BodyHandle addStaticBody(CollisionShape shape, double x, double y, double z,
        ...
        return bodyId == com.github.stephengold.joltjni.Jolt.cInvalidBodyId
                ? BodyHandle.INVALID
    ...
    public void syncStaticColliders(Engine engine) {
        ...
        destroyStaticBodies();
        com.moud.core.scene.SceneTree tree = engine.sceneTree();
    ...
    private int createBody(CollisionShape shape, double x, double y, double z,
        ...
        com.github.stephengold.joltjni.Shape joltShape = toJoltShape(shape);
        if (joltShape == null) {
            return com.github.stephengold.joltjni.Jolt.cInvalidBodyId;
        }
        try {
            ...
            if (motionType == EMotionType.Dynamic && mass > 0.0f) {
                settings.setMassPropertiesOverride(
                        new com.github.stephengold.joltjni.MassProperties().setMass(mass));
                settings.setOverrideMassProperties(
                        com.github.stephengold.joltjni.enumerate.EOverrideMassProperties.CalculateInertia);
            }
            try {
    ...
    private static com.github.stephengold.joltjni.Shape toJoltShape(CollisionShape shape) {
        if (shape == null) {
            return null;
        }
        try {
            return switch (shape) {
                case CollisionShape.Box b -> new com.github.stephengold.joltjni.BoxShape(
                        (float) Math.max(1e-6, b.halfX()),
                        (float) Math.max(1e-6, b.halfY()),
                        (float) Math.max(1e-6, b.halfZ()));
                case CollisionShape.Sphere s -> new com.github.stephengold.joltjni.SphereShape(
                        (float) Math.max(1e-6, s.radius()));
                case CollisionShape.Capsule c -> new com.github.stephengold.joltjni.CapsuleShape(
    ...
    private int createStaticBox(Transform world) {
        ...
        com.github.stephengold.joltjni.BoxShape shape =
                new com.github.stephengold.joltjni.BoxShape(sx * 0.5f, sy * 0.5f, sz * 0.5f);
        try {
            QuatD rot = world.rot();
            BodyCreationSettings settings = new BodyCreationSettings(
                    shape,
