...
public final class Stores {
    ...
    public static void install(LuaState state, StoreRef store) {
        global(state, "__moud_store_get", s -> {
            LuaJson.push(s, store.get(s.checkString(1), s.checkString(2)));
            return 1;
        });
        global(state, "__moud_store_set", s -> {
        ...
        global(state, "__moud_store_keys", s -> {
        ...
        global(state, "__moud_store_lease", s -> {
            s.pushBoolean(store.lease(s.checkString(1), s.checkString(2), s.checkString(3), s.checkNumber(4)));
            return 1;
        });
        global(state, "__moud_store_release", s -> {
            store.release(s.checkString(1), s.checkString(2), s.checkString(3));
            return 0;
        });
        global(state, "__moud_store_owner", s -> {
            s.pushString(store.owner());
            return 1;
        });
    }

    private static void global(LuaState state, String name, ToIntFunction<LuaState> body) {
