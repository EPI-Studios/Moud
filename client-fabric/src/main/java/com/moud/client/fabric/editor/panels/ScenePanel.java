...
public final class ScenePanel extends Panel {
    ...
    public void handleKey(UiContext ctx, KeyEvent e) {
        ...
        if (renamingNodeId >= 0) {
            ...
            if (e.isPressOrRepeat() && e.key() == com.miry.platform.InputConstants.KEY_ENTER) {
                commitInlineRename();
            } else if (e.isPressOrRepeat() && e.key() == com.miry.platform.InputConstants.KEY_ESCAPE) {
                renamingNodeId = -1;
            }
            return;
        }
        if (saveBranchOpen) {
            if (e.isPressOrRepeat() && e.key() == com.miry.platform.InputConstants.KEY_ESCAPE) {
        ...
        if (treeView != null && treeView.isFocused(ctx) && e.isPressOrRepeat()) {
            if (e.key() == com.miry.platform.InputConstants.KEY_F2) {
            ...
            if (e.key() == com.miry.platform.InputConstants.KEY_DELETE) {
                EditorState state = runtime.state();
                SceneSnapshot.NodeSnapshot selected = state != null ? state.scene.getNode(state.selectedId) : null;
