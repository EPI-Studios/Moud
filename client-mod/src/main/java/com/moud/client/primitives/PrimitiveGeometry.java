...
public final class PrimitiveGeometry {
    ...
    public record Triangle(Vector3 v0, Vector3 v1, Vector3 v2) {}

    private PrimitiveGeometry() {}

    public static List<Triangle> generateMesh(MoudPackets.PrimitiveType type, Vector3 pos, Quaternion rot, Vector3 scale,
                                              List<Vector3> customVerts, List<Integer> customInds) {
        ...
        switch (type) {
            case CUBE -> generateCube(triangles, pos, rot, scale);
            case SPHERE -> generateSphere(triangles, pos, rot, scale);
            case CYLINDER -> generateCylinder(triangles, pos, rot, scale);
            case CAPSULE -> generateCapsule(triangles, pos, rot, scale);
            case CONE -> generateCone(triangles, pos, rot, scale);
            case PLANE -> generatePlane(triangles, pos, rot, scale);
            case MESH -> {
                if (customVerts != null && !customVerts.isEmpty() && customInds != null && !customInds.isEmpty()) {
                    generateCustomMesh(triangles, pos, rot, scale, customVerts, customInds);
                }
            }
            default -> {}
        }
    ...
    public static Box computeBounds(List<Triangle> triangles) {
    ...
    private static void generateSphere(List<Triangle> triangles, Vector3 pos, Quaternion rot, Vector3 scale) {
        ...
        for (int r = 0; r <= rings; r++) {
            ...
            for (int s = 0; s < segments; s++) {
                ...
                grid[r][s] = transform(new Vector3((float) Math.cos(theta) * rRad, y, (float) Math.sin(theta) * rRad), pos, rot);
            }
        }
    ...
    private static void generateCapsule(List<Triangle> triangles, Vector3 pos, Quaternion rot, Vector3 scale) {
        ...
        generateHemisphere(triangles, pos, rot, radius, hh, hemiRings, true);  // Top
        generateHemisphere(triangles, pos, rot, radius, -hh, hemiRings, false); // Bottom
    }

    private static void generateHemisphere(List<Triangle> triangles, Vector3 pos, Quaternion rot, float radius, float yOffset, int rings, boolean isTop) {
        for (int r = 0; r < rings; r++) {
    ...
    private static void generateCustomMesh(List<Triangle> triangles, Vector3 pos, Quaternion rot, Vector3 scale,
                                           List<Vector3> verts, List<Integer> inds) {
        for (int i = 0; i + 2 < inds.size(); i += 3) {
            int i0 = inds.get(i), i1 = inds.get(i + 1), i2 = inds.get(i + 2);
